# Copy this file in config/initialiazers/find_missing_indexes.rb
...
class NoIndexDetector
  ...
  def resume(event)
    if @warnings.any?
      table = Terminal::Table.new headings: ["ms", "name", "query"], rows: @warnings.map { |w| [w.duration.round(1), w.name, w.query.chars.each_slice(80).map(&:join).join("\n")] }
